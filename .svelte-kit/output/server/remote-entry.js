...
import { a as flatten_issues, c as throw_on_old_property_access, n as create_field_proxy, o as normalize_issue, r as deep_set, s as set_nested_value } from "./chunks/form-utils.js";
	...
	const wrapper = (arg) => {
		...
		if (state.is_endpoint_request) {
			if (![
				...
				"PATCH",
				"DELETE"
			].includes(event.request.method)) throw new Error(`Cannot call a command (\`${__.name}(${maybe_fn ? "..." : ""})\`) from a ${event.request.method} handler`);
		} else if (!event.isRemoteRequest) throw new Error(`Cannot call a command (\`${__.name}(${maybe_fn ? "..." : ""})\`) during server-side rendering`);
		state.refreshes ??= {};
...
function form(validate_or_fn, maybe_fn) {
	...
	function create_instance(key) {
		...
		Object.defineProperty(button_props, "enhance", { value: () => {
			return {
				type: "submit",
		...
		Object.defineProperty(instance, "buttonProps", { value: button_props });
		const __ = {
			type: "form",
			name: "",
			id: "",
			fn: async (data, meta, form_data) => {
				...
				if (validated?.issues !== void 0) handle_issues(output, validated.issues, form_data);
					...
					try {
						output.result = await run_remote_function(event, state, true, data, (d) => d, (data$1) => !maybe_fn ? fn() : fn(data$1, issue));
					} catch (e) {
		...
		Object.defineProperty(instance, "fields", { get() {
			...
			return create_field_proxy({}, () => data?.input ?? {}, (path, value) => {
		...
		Object.defineProperty(button_props, "pending", { get: () => 0 });
...
function prerender(validate_or_fn, fn_or_options, maybe_options) {
	...
	const wrapper = (arg) => {
		const promise = (async () => {
			...
			const promise$1 = get_response(__, arg, state, () => run_remote_function(event, state, false, arg, validate, fn));
...
function batch(validate_or_fn, maybe_fn) {
	...
	const __ = {
		...
		run: (args) => {
			const { event, state } = get_request_store();
			return run_remote_function(event, state, false, args, (array) => Promise.all(array.map(validate)), fn);
		}
	};
	let batching = {
		args: [],
		...
		const get_remote_function_result = () => {
			return new Promise((resolve, reject) => {
				...
				setTimeout(async () => {
					...
					try {
						const get_result = await run_remote_function(event, state, false, batched.args, (array) => Promise.all(array.map(validate)), fn);
						for (let i = 0; i < batched.resolvers.length; i++) try {
							batched.resolvers[i].resolve(get_result(batched.args[i], i));
						} catch (error$1) {
							batched.resolvers[i].reject(error$1);
						}
					} catch (error$1) {
						for (const resolver of batched.resolvers) resolver.reject(error$1);
					}
				}, 0);
			});
		};
